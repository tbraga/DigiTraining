<?php
	...
	function elbrus_sanitize_text( $input ) {
		return wp_kses_post( force_balance_tags( $input ) );
	}

	function elbrus_sanitize_checkbox( $input ) {
		if ( $input == 'on' ) {
			return 'on';
		} else {
			return '';
	...
	function elbrus_sanitize_per_page( $input ) {
		if ( is_numeric( $input ) && $input != '0' ) {
			return absint( $input );
		} else return '';
	}

	function elbrus_sanitize_absinteger( $input ) {
		if ( is_numeric( $input ) && $input != '0' ) {
			return absint( $input );
		} else return '';
	}

	function elbrus_sanitize_email( $email ) {
		if (is_email( $email ) ) {
			return $email;
		} else {
			return '';
	...
	function elbrus_sanitize_layout( $value ) {
	...
	function elbrus_sanitize_loader( $value ) {
	...
	function elbrus_sanitize_onoff( $value ) {
	...
	function elbrus_sanitize_sidebar_blog_type( $value ) {
	...
	function elbrus_sanitize_sidebar_blog_content( $value ) {
		if ( ! in_array( $value, array( 'sidebar-1', 'global-sidebar-1', 'portfolio-sidebar-1', 'shop-sidebar-1', 'product-sidebar-1', 'custom-area-1' ) ) ) {
			$value = 'sidebar-1';
		}
	...
	function elbrus_sanitize_header_type( $value ) {
	...
	function elbrus_sanitize_overlay_opacity( $value ) {
	...
	function elbrus_sanitize_portfolio_type( $value ) {
	...
	function elbrus_sanitize_portfolio_perrow( $value ) {
	...
	function elbrus_sanitize_sidebar_portfolio_type( $value ) {
		if ( ! in_array( $value, array( '1', '2', '3' ) ) ) {
			$value = '2';
		}
	...
	function elbrus_sanitize_sidebar_portfolio_content( $value ) {
	...
	function elbrus_sanitize_animation( $value ) {
	...
	function elbrus_sanitize_footer_block($input) {
		$valid = elbrus_get_staticblock_option_array();
